...
// app.post('/poll-session', async (req, res) => {
//     try {
//         const response = await RunCollection(req, Three60ReviewCookies, collectionName);
//         res.json({ success: true, data: response });
//     } catch (error) {
//         res.status(500).json({ success: false, error: error.message });
//     }
// });
//360 Allows for reactivation after period of no use, determeind by poll-sesssion
//poll session seems to be 10, after 10 you get the notice to reactive your ression for 5 more minus for its expires


// let elapsedTime = 0;
// let status = false; // Define status variable outside of startPolling function
...
//     try {
//          const response = await RunCollection(req,Three60ReviewCookies,"activate_session"); // Wait for the sendIncident function to complete
//         res.json({success:true , data: response}); // Send the response back to the client
//     } catch (error) {
//         res.status(500).json({ success: false, error: error.message });
//     }
